...
const FeaturedTask = () => {
  ...
  useEffect(() => {
    fetch("https://b11a10-server-side-gaziraihan1.vercel.app/freelance/recent")
      .then((res) => res.json())
    ...
    <div className="my-8 md:my-12 lg:my-18 grid grid-cols-1 bg-gray-50 px-2 lg:px-4 rounded py-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-4 lg:gap-8">
      ...
      {featureTask <= 0 ? (
        ...
        featureTask.map((task) => (
          <div
            className="w-full border hover:border-2 px-2 py-4  flex flex-col justify-between rounded border-gray-200 transition hover:shadow-2xl "
            key={task._id}
          >
            <h2
              className={`${
            ...
            <Link
              ...
              className="mt-5 flex items-center border justify-center rounded-4xl cursor-pointer w-full py-2 px-4 text-gray-800"
            >
